class Board {
     ...
     * @param   {number}    safe_c  col of first click
     * @returns {void}
     */
    createBoard(safe_r, safe_c) {
        this.created = true;
        let arr = Array(CONFIG.rows).fill(0).map(() => Array(CONFIG.cols).fill(0));
        let choices = [];
        for (let r = 0; r < CONFIG.rows; r++) {
            for (let c = 0; c < CONFIG.cols; c++) {
                if (Math.abs(safe_r - r) <= CONFIG.saferadius && Math.abs(safe_c - c) <= CONFIG.saferadius) {
                    continue;
                }
                choices.push([r, c]);
            }
        ...
        for (let r = 0; r < CONFIG.rows; r++) {
            for (let c = 0; c < CONFIG.cols; c++) {
                for (let drc of CONFIG.adj) {
                    ...
                    if (new_r < 0 || new_r >= CONFIG.rows || new_c < 0 || new_c >= CONFIG.cols) {
                        continue;
                    }
    ...
    click(r, c, middleClick) {
        ...
        if (middleClick && this.arr[r][c].val === flagsAround && this.countCoveredAround(r, c) > flagsAround) {
     ...
     * @param   {number}    r   row
     * @param   {number}    c   col
     * @returns {boolean}   true if modified board
     */
    clickAround(r, c) {
        if (!Board.borderCheck(r, c)) {
            return false;
        ...
        for (let [newR, newC] of this.arr[r][c].adj) {
            if (!Board.borderCheck(newR, newC) || !this.arr[newR][newC].covered || this.arr[newR][newC].flagged) {
                continue;
            }
